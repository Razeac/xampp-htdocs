...
function create_section($cshort,$cdate){
		
	if($cshort==""){
 
echo "<script>alert('Select Section Name')</script>";
...
$db = Database::getInstance();
$mysqli = $db->getConnection();
$query = "insert into grsec (name) values (?)";
$stmt= $mysqli->prepare($query);
if(false===$stmt){
...
function showSection(){
	...
	$db = Database::getInstance();
	$mysqli = $db->getConnection();
	$query = "SELECT * FROM grsec ";
	$stmt= $mysqli->query($query);
	return $stmt;
	
}

function showSection1($cid){
	
	$db = Database::getInstance();
	$mysqli = $db->getConnection();
	$query = "SELECT * FROM grsec  where id='".$cid."'";
	$stmt= $mysqli->query($query);
	return $stmt;
}
...
function register($cshort,$cfull,$fname,$mname,$lname,$gender,$gname,$ocp,$income,$category,$ph,
   ...
   $query = "INSERT INTO `registration` (`course`, `subject`, `fname`, `mname`, `lname`, `gender`, `gname`, `ocp`,
			...
			$stmt->bind_param('sssssssssssssssssssssssssssssssss',
		         	$course,$cfull,$fname,$mname,$lname,$gender,$gname,$ocp,$income,$category,$ph,$nation,$mobno,
...
function edit_section($cshort,$id){

    $db = Database::getInstance();
	$mysqli = $db->getConnection();
	//echo $cshort.$cfull.$udate.$id;exit;
	$query = "update grsec set name=? where id=?";
	$stmt= $mysqli->prepare($query);
	$stmt->bind_param('sssi', $cshort, $id);
	$stmt->execute();
    echo '<script>'; 
    echo 'alert("Section Updated Successfully")'; 
    echo '</script>';
...
function edit_std($course,$cfull,$fname,$mname,$lname,$gender,$gname,$ocp,$income,$category,$ph,
                  $nation,$mobno,$email,$country,$state,$city,$padd,$cadd,$board1,$board2,$roll1,$roll2,
  ...
  // echo $id;exit;
	...
	$rc=$stmt->bind_param('sssssssssssssssssssssssssssssssi',$course,$cfull,$fname,$mname,$lname,$gender,$gname,$ocp,$income,$category,$ph,
                  $nation,$mobno,$email,$country,$state,$city,$padd,$cadd,$board1,$board2,$roll1,$roll2,
				   $pyear1,$pyear2,$sub1,$sub2,$marks1,$marks2,$fmarks1,$fmarks2,$id);
...
function del_section($id){
	...
	//  echo $id;exit;
	 $db = Database::getInstance();
	 $mysqli = $db->getConnection();
	 $query="delete from grsec where id=?";
	 $stmt= $mysqli->prepare($query);
	 $stmt->bind_param('s',$id);
	 $stmt->execute();
